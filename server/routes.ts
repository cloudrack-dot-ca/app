...
export async function registerRoutes(app: Express): Promise<HttpServer> {
  ...
  app.patch("/api/docs/sections/:id/reorder", async (req, res) => {
    ...
    try {
      ...
      await db.update(schema.docSections)
        .set({ order })
        .where(eq(schema.docSections.id, sectionId));

      // Get the updated section
      const updatedSection = await db.query.docSections.findFirst({
        where: eq(schema.docSections.id, sectionId)
      });
  ...
  // Reorder documentation article
  app.patch("/api/docs/articles/:id/reorder", async (req, res) => {
    if (!req.user) return res.sendStatus(401);

    try {
      const articleId = parseInt(req.params.id);
      const { order, sectionId } = req.body;
      ...
      if (typeof sectionId !== 'number') {
        return res.status(400).json({ message: "Section ID must be a number" });
      }

      // Update the article's order and section
      await db.update(schema.docArticles)
        .set({ 
          order,
          sectionId,
          lastUpdated: new Date()
      ...
      const updatedArticle = await db.query.docArticles.findFirst({
        where: eq(schema.docArticles.id, articleId)
      });
      ...
      res.status(500).json({ message: "Failed to reorder article" });
    }
  });
