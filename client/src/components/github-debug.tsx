...
Scope: ${response.url.match(/scope=([^&]+)/)?.[1] || "Not found"}
  ...
  const handleDirectAuth = () => {
    // Manual OAuth URL with correct client ID from the environment info
    const clientId = envVars?.env?.GITHUB_CLIENT_ID || "Ov23lis2zEGGv7CCm9SG";
    const redirectUri = envVars?.env?.GITHUB_REDIRECT_URI || "http://localhost:5000/api/github/callback";
